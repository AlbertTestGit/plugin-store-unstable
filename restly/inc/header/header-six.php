<?php 
$restly_show_cta6 = restly_options('restly_show_cta6');
$restly_show_minicart = restly_options('restly_show_minicart');
		...
		<div class="container">
			<nav class="navbar navbar-expand-lg navbar-light main-navigation" id="site-navigation">
				<div class="logo-area">
					<div class="site-branding">
						...
						if(!empty(is_array($restlyMeta) && array_key_exists('restly_meta_select_logo', $restlyMeta)) && $restlyMeta['restly_meta_select_logo'] == true && !empty($restlyMeta['restly_meta_logo']['url'])){
							...
							$restly_ShowLogo = restly_options('restly_show_hlogo6');
							$restly_logo = restly_options('restly_logo6');
				...
				<div class="navbar-collapse nav-menu stellarnav">
					...
					?>
					<?php 
					if($restly_show_search6 == true ){
						get_template_part('inc/header/search','button');
					}
					if ( class_exists( 'WooCommerce' ) && $restly_show_minicart == true ) {
							restly_woocommerce_header_cart();
					}
					if($restly_show_login6 == true){
						get_template_part('inc/header/login','button');
					}
					?>
				</div>
			</nav>
